...
class ListModule(nn.Module):
    ...
    def __getitem__(self, idx):
        if idx > len(self._modules()):
            raise IndexError("idex {} is out of range".format(idx))
        if idx < 0:
...
class UNet(nn.Module):
    '''
    upsample_model in ['deconv', 'nearest', 'bilinear']
    ...
    def __init__(self, num_input_channels=3, num_output_channels=3, feature_scale=4,
        ...
        self.start = unetConv2(num_input_channels, filters[0] if not concat_x else filter[0] - num_input_channels, norm_layer, need_bias, pad)
    ...
    def forward(self, inputs):
        #Downsample
        print('unet forward:')
        print('unet forward inputs.size():', inputs.size())
        downs = [inputs]
        down = nn.AvgPool2d(2, 2)
        for i in range(4 + self.more_layers):
            downs.append(down(downs[-1]))
        print('class UNet: forward:inputs.size()', inputs.size())
        in64 = self.start(inputs)
        print('class UNet: forward:in64.size()', in64.size())
        if self.concat_x:
        ...
        print('UNet: forward:up1.size()', up1.size())
        print('UNet: forward:self.final(up1).size()', self.final(up1).size())
...
class unetDown(nn.Module):
    ...
    def forward(self, inputs):
        print('UNet class unetDown:forward:inputs.size()', inputs.size())
        outputs = self.conv(inputs)
        outputs = self.down(outputs)
        print('UNet class unetDown:forward:outputs.size()', outputs.size())
        return outputs
...
class unetUp(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        print('UNet: class unetUp: forward: inputs1.size()', inputs1.size())
        in1_up = self.up(inputs1)
        print('UNet: class unetUp: forward: in1_up=self.up(inputs1).size()', in1_up.size())
